...
We will need the following restricted invertibility.

\begin{theorem}\label{thm:rest-inv}
Let $Q \in \R^{n \times n}$ be positive definite quadratic form and $\eps \in
(0,1)$. Let $\lambda_1 > 0$ denote the maximum eigen value of $Q$.  For $k =
\ceil{\eps^2 \tr(Q)/\lambda_1}$, there exists $S \subseteq [n]$, $|S|=k$, such
that $\lambda_{\rm min}(Q_{S,S}) \geq \frac{(1-\eps)^2\tr(Q)}{n}$, where
$\lambda_{\rm min}(Q_{S,S})$ is the minimum eigen value of $Q_{S,S}$.
\end{theorem}
...
\max \set{r \geq 0: r K \cap W_S \subseteq k \conv\set{\pm u_i: i \in S}}.
\]
Then, we have that
\begin{equation}
\label{eq:spec-lb}
...
\]
Since $u_x \in \beta K$ and $r K \subseteq n \conv\set{\pm u_i: i
\in [n]}$, we have that
\begin{align*}
\alpha n \leq \beta \max_{z \in K} \inner{v_x}{z} 
         \leq \frac{\beta}{r} n 
               \max_{z \in \conv\set{\pm u_i: i \in [n]}} \inner{v_x}{z}
          = \frac{\beta}{r} n .
\end{align*}
...
Two natural weakenings of Conjecture~\ref{conj:rip-convex} are to ask
...
Conjecture~\ref{conj:rip-convex}. In particular, when attempting to prove
...
missing for the proof of Conjecture~\ref{conj:rip-convex} is a
