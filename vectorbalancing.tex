...
$+1$ and $-1$ elements in every set? The tools developed for deriving bounds on
...
to communication complexity, and most recently, to approximation algorithms. 
...
constructive methods more robust, i.e.~the conditions under which such ``fractional''
...
net of $C$. For question 1a, one may be tempted to ask whether we can directly
...
starting from $U \in \R^{m \times k}$, with $\rank(U) = n$ and rank
...
polytope with $m$ facets. Thus, for any $B \in \R^{n \times k}$, one can
equivalently restate the guarantees of~\cite{Bansal10} as yielding colorings of
discrepancy $O(\sqrt{\log m \log n} \hd(B,K))$ and of~\cite{disc-gamma2} as a
$O(\sqrt{\log m}\log n)$ approximation to $\hd(B,K)$ for any $n$-dimensional
...
approximations are possible. In particular, we give a polynomial time algorithms for
computing colorings of discrepancy $O(\log n \hd(B,K))$ and approximating
$\hd(B,K)$ up to $O(\log^{2.5} n)$ factor, assuming $K$ is represented by an
appropriate oracle.  For this purpose, we show that Rothvoss' partial coloring
algorithm~\cite{rothvoss-giann} is nearly optimal for hereditary discrepancy in
general norms, and provide a substantially tighter factorization
approach~\cite{disc-gamma2} for hereditary discrepancy. As part of our analyzes,
we give novel methods for relating volumes to Gaussian measure, which may be of
