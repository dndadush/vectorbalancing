...
% should put all prelims in one place
% add prelims for dual ops, Schur convexity
...
A linear operator $A: X \to X^*$ on normed space $X$ defines a bilinear
form $B$ on $X \times X$, given by $B(x,y) = \langle x, Ay
...
\in X$, and $\langle x, Ax \rangle >0$
for all nonzero $x \in X$. In the case of $\R^n$ this is equivalent to
stating that the matrix $M$ of $A$ with respect to the standard basis
is positive definite, i.e.~is symmetric and all its eigenvalues
are positive. 

For a positive definite operator $A:\ell_2^n \to \ell_2^n$ there
exists a unique positive definite operator $B:\ell_2^n \to \ell_2^n$
such that $B^*B = BB^* = B^2 = A$. We use the notation $A^{1/2}$ for
$B$. The matrix of $A^{1/2}$ equals the usual matrix square root of
the matrix $A$ (both with respect to the standard basis). We also use
the shorthand notation $A^{k/2} \eqdef (A^k)^{1/2}$ for (positive or
definite) integers $k$.
...
\begin{theorem}\label{thm:factorization}
  ...
  \to Y$ between two normed spaces $X, Y$ over $\R^n$, we have
  \[
  \vb(U) \le C\lambda(U).
  \]
\end{theorem}
...
direction as well, up to a poly-logarithmic factor, i.e.~to show that
$\lambda(U) \le (\log n)^{O(1)}\vb(U)$. Moreover, we will show that,
under reasonable assumptions, $\lambda(U)$ can be computed
efficiently.
...
$\lambda(U)$, which we then use to show that the upper bound in
Theorem~\ref{thm:factorization} is approximately tight.
...
Next we give a dual formulation of $\lambda(U)$ as a
...
\begin{theorem}\label{thm:dual}
  ...
  v_1, \ldots, \pm v_m\}$. Then $\lambda(U)$ equals
  \begin{align}
...
\begin{proof}
  ...
  the equation $\nabla f(A) = Q$ has a solution over $A \succ 0$, and
  $f^*(Q)$ is achieved at this solution. Let $B$ be the solution, and
  let $R:Y \to \ell_2^n$ be the invertible map such that $\ell^*(R)
  \le 1$ and $f(A) = \tr((RUA^{-1}U^*R^*)^{1/2})$. Let
  $B = RU$; $\nabla f(A) = Q$ implies
  \[
  (B^{-*} A B^{-1})^{-3/2} = -2 BQB^*,
  \]
  ...
  \lambda(U) = 
  \sup\left\{-\sum_{i = 1}^m{q_i} 
    + \frac{3}{2^{2/3}} \tr((-(RU(\sum_{i = 1}^m{q_i  v_i\otimes v_i})U^*R^*)^{1/3}):
    R:Y \to \ell_2^n,\ell^*(R) \le 1,
    q_1, \ldots, q_m \ge 0\right\}.
  \]
  ...
  \lambda(U) = 
  \sup\left\{-t + 
    \frac{3t^{1/3}}{2^{2/3}} \tr((-(RU(\sum_{i = 1}^m{p_i  v_i\otimes v_i})U^*R^*)^{1/3}):
...
function
\begin{align}
  f(X) \eqdef &\inf \tr((UA^{-1}U^*)^{1/2}) \label{eq:single-obj}\\
  &\text{s.t.}\notag\\
  &A: X \to X^*,\|A\| \le 1\label{eq:single-constr}\\
  &A \succ 0.\label{eq:single-psd}.
\end{align}
...
function $\tr((UA^{-1}U^*)^{1/2})$ and the constraints
\eqref{eq:single-constr}--\eqref{eq:single-psd} are convex in $A$. We
...
\begin{proof}
  ...
  \begin{align}
    f(X) \eqdef &\inf \tr((UA^{-1}U^*)^{1/2}) \label{eq:sing2-obj}\\
    &\text{s.t.}\notag\\
    &\langle v_i, Av_i\rangle \le 1 \ \ \forall i \in [m]\label{eq:sing2-constr}\\
    &A \succ 0\label{eq:sing2-psd}.
  \end{align}
  
  Define the function $g(A) = \tr((UA^{-1}U^*)^{1/2})$ on positive
  definite $A$. Recall that the conjugate function $g^*$ is defined
  by:
  \[
  g^*(Q) \eqdef \sup\{\tr(QA) - g(A): A \succ 0\}.
  \]
...
\[
\lambda(U) = 
\inf_{A \in \mathcal{D}_X} \sup_{R \in \mathcal{D}_X} 
\tr(RUA^{-1}U^*R^*)^{1/2}),
\]
where $\mathcal{D}_X \eqdef \{A: X \to X^*, \|A\|\le 1, A \succ 0\}$
...
\begin{equation}
  ...
  \tr(RUA^{-1}U^*R^*)^{1/2})
  = 
  \sup_{R \in \mathcal{D}_X} \inf_{A \in \mathcal{D}_X} 
  \tr(RUA^{-1}U^*R^*)^{1/2}).
\end{equation}
...
\subsection{Characterizing $\beta(X,Y)$ in terms of $\lambda(U)$}
...
\begin{theorem}\label{thm:fact-vollb}
  ...
  \[
  \lambda(U) \le C K(Y) (1 + \log n)^{3/2} \vollb((v_i)_{i =
    1}^m, Y),
  \]
  where $U:X \to Y$ is the formal identity operator, and $K(Y)$
