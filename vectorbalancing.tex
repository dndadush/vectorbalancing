...
While Theorem~\ref{thm:tightness} gives an efficient method of
balancing a given set of vectors approximately optimally, it does not
give an efficiently computable upper bound of the vector balancing
constant $\vb(C,K)$ of two symmetric convex bodies $C$ and $K$, or of
the hereditary discrepancy $\hd((u_i)_{i = 1}^N, K)$. Moreover, we are
not aware of an efficient algorithm for computing the volume lower
bound, which is itself a maximum over an exponential number of
volumes. Instead we explore a different approach, which
relies on applying Banaszczyk's vector balancing theorem in an optimal
way in order to get an efficiently computable, and nearly tight, upper
bound on both vector balancing constants and hereditary
discrepancy. 
...
5$. The theorem gives an upper bound on $\vb(B_2^n, K)$ in terms
of the Gaussian measure of the body $K$. In particular, if $r$ is the
smallest number such that the Gaussian measure of $rK$ is $\frac12$,
then the theorem tells us that $\vb(B_2^n, K) \le 5r$. To use this
upper bound for bodies $C$ different from $B_2^n$, we find an
optimal way to map $C$ into $B_2^n$ and then use the theorem as
above. It turns out that the most convenient language for for this
approach is that of linear operators between normed spaces. In
particular, we recast the problem of mapping $C$ into $B_2^n$ as one
of factoring a linear operator through Euclidean space. This
factorization strategy is a significant generalization of the
connection between the $\gamma_2$ norm and hereditary discrepancy used
in~\cite{disc-gamma2}.

Let us now introduce the generalization of vector balancing constants
to operators. 
...
As a warm-up, let us see how we can use this theorem to get
