...
\subsection{Nearly Optimal Approximations for Vector Balancing and Hereditary
Discrepancy}
...
over to vector balancing, and thus we will mostly focus on the former. For
question 1a, one may be tempted to ask whether we can directly compute a
coloring whose $K$-discrepancy is approximately $\disc((u_i)_{i=1}^N,K)$ instead
of $\hd((u_i)_{i=1}^N,K)$. Unfortunately, even for $K = B_\infty^n$ and
$(u_i)_{i=1}^n \in [-1,1]^n$, it is NP-hard to distinguish whether
$\disc((u_i)_{i=1}^n,B_\infty^n)$ is $0$ or $\Omega(\sqrt{n})$ (note that
$O(\sqrt{n})$ is guaranteed by Spencer's theorem), thus one cannot hope for any
non-trivial approximation guarantee in this context. We now discuss prior work
on these questions and then continue with our main results. 
...
\]
where $\|A\|_{2 \rightarrow 1}$ is the maximum $\ell_2$ norm of any row of $A$,
...
hereditary discrepancy. For the context of $\ell_2$, it is in fact not hard to
extract from the techniques of~\cite{NT15,disc-gamma2} a $O(\log
\rank(U))$-approximation to $\hd(U,B_2^m)$. In essence, all of the above
techniques implicitly replace the $\ell_\infty$ norm via an averaged version of
$\ell_2$, where one optimizes over the averaging coefficients, which makes the
$\ell_2$ norm by itself an easier special case of the analysis
in~\cite{NT15,disc-gamma2}.
...
symmetric polytope $K$ with $m$ facets. Thus, the main question is whether there
exist corresponding approximation algorithms which depend only
polygarithmically on the dimension of the norm and not on the complexity its
representation. 
...
\log n})$ and $O(\sqrt{n}\log n)$ approximations respectively. Interestingly,
these guarantees are identical to what can be achieved by replacing $K$ by a
symmetric polytope with $3^n$ facets, which can achieve a sandwiching factor of
$2$, and applying the $\ell_\infty$ results.

\paragraph{\bf Results.} Our main results are that indeed such polylogarithmic
approximations are possible. In particular, for $U \in \R^{n \times N}$ and $K
\subseteq \R^n$ a symmetric convex body, we give randomized polynomial time
algorithms for computing colorings of discrepancy $O(\log n \hd(U,K))$ and
approximating $\hd(U,K)$ up to $O(\log^{2.5} n)$ factor, assuming $K$ is
represented by an appropriate oracle. Furthermore, if $K$ is a polyhedral norm
with at most $m$ facets, our approximation algorithm for $\hd(U,K)$ always does
better than the $\gamma_2$ approach, and hence achieves an $O(\min \set{\log n
\sqrt{\log m}, \log^{2.5} n})$ approximation. To achieve these results, we show
that Rothvoss' partial coloring algorithm~\cite{rothvoss-giann} is nearly
optimal for hereditary discrepancy in general norms by showing near-tightness
with respect to a known volumetric lower bound, and we provide a substantially
tighter factorization approach~\cite{disc-gamma2} for hereditary discrepancy.
Our analyzes rely on combining tools from convex geometry and discrepancy in a
novel way. In particular, we give a new way to prove lower bounds on Gaussian
measure using only volumetric information, which could be of independent
interest.  Furthermore, we make a natural geometric conjecture which would imply
that Rothvoss' algorithm is some sense optimal for finding partial colorings in
any norm, and prove the conjecture for the special case of $\ell_2$.

Comparing to prior work, our coloring algorithm and hereditary discrepancy
approximation algorithm give uniformly better (or at at least no worse)
guarantees in every setting which has been studied\footnote{To recover Larsen's
hereditary guarantee for $\ell_2$ coloring, we require a minor modification of
Rothvoss' algorithm}. Our methods provide a unified approach for studying
discrepancy in arbitrary norms, and have no dependence on the description
complexity of the output norm.

As mentioned above, our approximate characterization of hereditary discrepancy
transfers seemlessly to the vector balancing setting. The slightly more accurate
statement is that neither the volumetric lower bound we rely upon nor our
factorization based upper bound ``see'' the difference between vector balancing
and hereditary discrepancy, that is both bounds remain invariant when replacing
$\hd(U,K)$ and $\vb(\conv(\pm u_i: i \in [N]),K)$, where we note that clearly
$\hd(U,K) \leq \vb(\conv(\pm u_i: i \in [N]),K)$. This has the interesting and
rather non-obvious implication that $\vb(\conv(\pm u_i: i \in [N]),K) \leq
O(\log n) \hd(U,K)$. 
...
hereditary discrepancy. Given a sequence of vectors $(u_i)_{i = 1}^N$
in $\R^n$ and convex body $K$ in $\R^n$, we can define $X$ to be the
normed space with unit ball $B_X = \conv\{\pm u_1, \ldots \pm u_N\}$,
and $Y$ to be the normed space with unit ball $K$. 
...
m\}$ so that the Gram matrix of the vectors $\{RUx_i: i \in S\}$ has large
...
\begin{proof}[Proof of Theorem~\ref{thm:fact-vollb}]
  ...
  \[
  \det(G_{S,S})^{1/2} 
  = \frac{\vol_k(RUV(B_2^S))}{\vol_k(B_2^n \cap \R^S)}
...
\section{Proofs of Convexity and Duality}
...
\begin{proof}%proof of lm:fact-convex
