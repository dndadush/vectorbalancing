...
constraints faster). As this only gives probability error guarantees for each
constraint in isolation, a union bound is used to get a global guarantee,
incurring a $O(\sqrt{\log m})$ dependence. 
...
measure (see Theorem~\ref{thm:roth-giann}), Rothvoss shows that $x$ has at least
...
it does need an explicit description of $K$), it is apriori unclear why Gaussian
measure should be large enough in the present context. 

Our main technical result, is to show that under condition~\ref{eq:big-volume},
there exists a section $K$ of dimension nearly $n$ whose Gaussian measure is at
least $2^{-O(n)}$ (see Theorem~\ref{thm:gauss-via-volume} for a precise
statement). 

This notion of ``largeness'' in fact suffices for Rothvoss's partial
coloring algorithm. From here, we simply apply it $O(\log n)$ times to yield the
desired full coloring. 

We note that the discrepancy guarantees are a natural byproduct of the
algorithm, and do not need to be explicitly enforced as in Bansal's algorithm.  
