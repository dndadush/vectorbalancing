...
respect to a volumetric lower bound of Banaszczyk~\cite{Bana93}, and we provide
a substantially tighter factorization approach~\cite{disc-gamma2} for hereditary
discrepancy / vector balancing. To analyze these approaches we rely on a novel
combination of tools from convex geometry and discrepancy. In particular, we
give a new way to prove lower bounds on Gaussian measure using only volumetric
information, which could be of independent interest. Furthermore, we make a
natural geometric conjecture which would imply that Rothvoss' algorithm is some
sense optimal for finding partial colorings in any norm, and prove the
conjecture for the special case of $\ell_2$.
...
\]
which is essentially tight. The above example gives hope that the volume lower
bound can circumvent any dependency on the facet complexity of the norm. The
next theorem shows that indeed this is the case:
...
also be shown to be tight. In particular, it can be shown that for 
the 3-permutations example from~\cite{NNN12}, which has $\ell_\infty$
discrepancy $\Omega(\log n)$, the volume lower bound is $O(1)$. The computations for
this are somewhat technical, so we defer a detailed discussion to the full
version. As mentioned previously, an interesting property is that it remains
invariant under taking convex hulls, namely $\vollb^{\rm h}(\conv(\pm U),K) =
\vollb^{\rm h}(U,K)$. This is proved in section~\ref{sec:conv-hulls}, using a
theorem of Ball~\cite{Ball88} to show that the volume lower bound is essentially
convex, and hence maximized at extreme points.

Our proof of Theorem~\ref{thm:tightness} is algorithmic and relies on Rothvoss'
constructive version of the vector balancing theorem of
Giannopoulos~\cite{giann}. These theorems directly rely on Gaussian measure
estimates of the feasible region and avoid having to argue about the discrepancy
incurred by each ``facet'' independently. We recall that the approach of
Bansal~\cite{Bansal10} is to initiate a random walk for which the variance of
the increments in the direction of each facet can be controlled by hereditary
discrepancy. Due to this local view, and potentially complex dependencies
between constraints, one is left with little choice but to do a union bound over
all constraints, yielding $O(\sqrt{\log m})$ type dependencies when $m$ is the
number of constraints. While the Gaussian measure approaches can avoid these
problems, one is left with the non-trivial task of showing that the relevant
Gaussian measures are indeed large. We explain our approach to this presently.
